...
#             print("validated data in services")
...
#             print(" --- Update visa_applications in services ---")
#             cursor.execute('''
...
class VisaService:
    ...
    def create_application(self, request: CreateApplicationRequest) -> CreateApplicationResponse:
        ...
        try:
            ...
            ''', (
                ...
                request.applicant.nationality,
                request.applicant.passport_number,
                request.applicant.passport_expiry,
                request.applicant.date_of_birth
            ))
            ...
            ''', (
                ...
                request.travel_details.destination_country,
                request.travel_details.purpose_of_travel,
                request.travel_details.departure_date,
                request.travel_details.return_date,
                request.travel_details.duration_days
            ))
    ...
    def get_application(self, application_id: str) -> Optional[VisaApplication]:
        ...
        try:
            ...
            cursor.execute('''
                SELECT id, visa_type, status, approval_status, current_stage,
                    current_approver, created_at, updated_at, submitted_at, notes
                FROM visa_applications WHERE id = %s
            ''', (application_id,))
            app_row = cursor.fetchone()
            ...
            cursor.execute('''
                SELECT employee_id, first_name, last_name, email, department,
                    position, nationality, passport_number, passport_expiry, date_of_birth
                FROM applicants WHERE application_id = %s
            ''', (application_id,))
            applicant_row = cursor.fetchone()
            ...
            cursor.execute('''
                SELECT destination_country, purpose_of_travel, departure_date,
                    return_date, duration_days
                FROM travel_details WHERE application_id = %s
            ''', (application_id,))
            travel_row = cursor.fetchone()
            ...
            cursor.execute('''
                SELECT approver, role, action, timestamp, comments
                FROM approval_history WHERE application_id = %s
                ORDER BY timestamp ASC
            ''', (application_id,))
            approval_rows = cursor.fetchall()
            ...
            application_data = {
                ...
                "applicant": {
                    ...
                    "nationality": applicant_row[6],
                    "passport_number": applicant_row[7],
                    "passport_expiry": applicant_row[8],
                    "date_of_birth": applicant_row[9]
                } if applicant_row else None,
                "travel_details": {
                    "destination_country": travel_row[0],
                    "purpose_of_travel": travel_row[1],
                    "departure_date": travel_row[2],
                    "return_date": travel_row[3],
                    "duration_days": travel_row[4]
                } if travel_row else None,
                "documents": [
                    {
            ...
            self.logger.info(f"Retrieved application {application_id}")
            return VisaApplication(**application_data)
    ...
    def update_application(self, application: VisaApplication) -> bool:
        ...
        try:
            ...
            cursor.execute('''
                UPDATE visa_applications
                SET visa_type = %s, status = %s, approval_status = %s, current_stage = %s,
                    current_approver = %s, notes = %s, updated_at = %s, submitted_at = %s
                WHERE id = %s
            ''', (
                application.visa_type,
                application.status,
            ...
            cursor.execute('''
                UPDATE applicants
                SET employee_id = %s, first_name = %s, last_name = %s, email = %s, department = %s,
                    position = %s, nationality = %s, passport_number = %s, passport_expiry = %s, date_of_birth = %s
            ...
            ''', (
                ...
                application.applicant.nationality,
                application.applicant.passport_number,
                application.applicant.passport_expiry,
            ...
            cursor.execute('''
                UPDATE travel_details
                SET destination_country = %s, purpose_of_travel = %s, departure_date = %s, return_date = %s, duration_days = %s
                WHERE application_id = %s
            ''', (
                application.travel_details.destination_country,
                application.travel_details.purpose_of_travel,
                application.travel_details.departure_date,
                application.travel_details.return_date,
                application.travel_details.duration_days,
                application.id
            ))
 
            # Update documents
            cursor.execute('DELETE FROM documents WHERE application_id = %s', (application.id,))
            for doc in application.documents:
                cursor.execute('''
                    INSERT INTO documents (application_id, document_name, document_url)
                    VALUES (%s, %s, %s)
                ''', (application.id, doc.filename, doc.file_path))
        ...
        try:
            ...
            if updates:
                ...
                if fields:
                    ...
                    values.append(application_id)
                    cursor.execute(f"UPDATE visa_applications SET {', '.join(fields)} WHERE id = %s", values)
            ...
            if "applicant" in updates:
                ...
                for key in ["employee_id", "first_name", "last_name", "email", "department", "position", "nationality", "passport_number", "passport_expiry", "date_of_birth"]:
                    if key in applicant:
                        applicant_fields.append(f"{key} = %s")
                        applicant_values.append(applicant[key])
                if applicant_fields:
                    applicant_values.append(application_id)
                    cursor.execute(f"UPDATE applicants SET {', '.join(applicant_fields)} WHERE application_id = %s", applicant_values)
            ...
            if "travel_details" in updates:
                ...
                for key in ["destination_country", "purpose_of_travel", "departure_date", "return_date", "duration_days"]:
                    if key in travel:
                        travel_fields.append(f"{key} = %s")
                        travel_values.append(travel[key])
                if travel_fields:
                    travel_values.append(application_id)
                    cursor.execute(f"UPDATE travel_details SET {', '.join(travel_fields)} WHERE application_id = %s", travel_values)
 
            if "documents" in updates:
                cursor.execute("DELETE FROM documents WHERE application_id = %s", (application_id,))
                for doc in updates["documents"]:
                    cursor.execute("INSERT INTO documents (application_id, document_name, document_url) VALUES (%s, %s, %s)",
                                 (application_id, doc["document_name"], doc["document_url"]))
    ...
    def update_embassy_status(self, application_id: str, request_data: dict) -> bool:
        ...
        try:
            ...
            # Check if embassy record exists
            cursor.execute('SELECT application_id FROM embassy_tracking WHERE application_id = %s', (application_id,))
            existing = cursor.fetchone()
            ...
            if existing:
                ...
                cursor.execute('''
                    UPDATE embassy_tracking
                    SET status = %s, embassy_name = %s, tracking_number = %s,
                        submission_date = %s, last_updated = %s, notes = %s
                    WHERE application_id = %s
                ''', (
                    status,
                    request_data.get('embassy_name'),
    ...
    def get_embassy_status(self, application_id: str) -> Optional[dict]:
        ...
        try:
            ...
            cursor.execute('''
                SELECT status, embassy_name, tracking_number, submission_date, last_updated, notes
                FROM embassy_tracking WHERE application_id = %s
            ''', (application_id,))
    ...
    def validate_document_content_enhanced(self, application_id: str) -> DocumentValidationResult:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                'SELECT document_name, document_url FROM documents WHERE application_id = %s',
                (application_id,)
            )
            table_documents = cursor.fetchall()
            conn.close()
    ...
    def _validate_application(self, request):
        """Validate application (Pydantic handles most validation)."""
        # Additional business logic validation
        if not request.visa_type:
            raise ValueError("Visa type is required")
       
        # Check passport validity for travel dates
        months_until_travel = (request.travel_details.departure_date - date.today()).days / 30
        months_until_passport_expiry = (request.applicant.passport_expiry - date.today()).days / 30
